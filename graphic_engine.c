...
void graphic_engine_paint_game(Graphic_engine *ge, Game *game) {
  ...
  if (id_act != NO_ID) {
    ...
    if (id_west != NO_ID) {
      ...
      strcpy(str3, temp);
      sprintf(temp, " |%s         |   %s", space_get_n_description(game_get_space(game, id_west), 0), str4);
      strcpy(str4, temp);
      sprintf(temp, " |%s         | < %s", space_get_n_description(game_get_space(game, id_west), 0), str5);
      strcpy(str5, temp);
      sprintf(temp, " |%s         |   %s", space_get_n_description(game_get_space(game, id_west), 0), str6);
      strcpy(str6, temp);
      sprintf(temp, " |%s         |   %s", space_get_n_description(game_get_space(game, id_west), 0), str7);
      strcpy(str7, temp);
  ...
  for (i = 0; i < game_get_num_objects(game); i++) {
    Object *obj = game_get_object_from_position(game, i);
    if (obj == NULL) {
  ...
  for (i = 0; i < game_get_num_objects(game); i++) {
    obj = game_get_object_from_position(game, i);
    if (obj == NULL) {
      printf("Object %d is NULL\n", i); /*  Debug statement */
      continue;
    }
