...
Status game_load_everything(Game *game, char *filename) {
  ...
  while (fgets(line, WORD_SIZE, file)) {
    ...
    if (strncmp("#s:", line, 3) == 0) {
      ...
      toks = strtok(NULL, "|");
      char space_image[WORD_SIZE] = "";
      if (toks) strcpy(space_image, toks); // Read space image path
      ...
      if (space) {
          ...
          space_set_image(space, space_image); // Store space image
      ...
      toks = strtok(NULL, "|");
      if (toks) strcpy(player_image, toks); // Read player image path
      ...
      if (player) {
          ...
          player_set_image(player, player_image); // Store player image
