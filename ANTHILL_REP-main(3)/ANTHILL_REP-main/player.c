...
/* Set player image */
Status player_set_image(Player *player, const char *image) {
  if (!player || !image) return ERROR;
  strncpy(player->image, image, MAX_MESSAGE - 1);
  player->image[MAX_MESSAGE - 1] = '\0';
  return OK;
}

/* Get player image */
const char *player_get_image(const Player *player) {
  if (!player) return NULL;
  return player->image;
}


int player_get_x(Player *player) { return player->x; }
int player_get_y(Player *player) { return player->y; }
