...
char *cmd_to_str[N_CMD][N_CMDT] = {
    {"", "No command"}, {"", "Unknown"},    {"e", "Exit"},   {"m", "Move"},
    {"t", "Take"},      {"d", "Drop"},      {"a", "Attack"}, {"c", "Chat"},
    {"i", "Inspect"},   {"n", "Inventory"}, {"e", "Use"},    {"o", "Open"}};
char *conn_to_str[N_CON][N_CMDT] = {
    {"over", "OVER"}, {"with", "WITH"}, {"", ""}};
...
Status command_get_user_input(Command *command) {
  ...
  for (i = 0; i < N_CMD; i++) {
    if ((cmd_to_str[i][CMDS] && !strcasecmp(token, cmd_to_str[i][CMDS])) ||
        (cmd_to_str[i][CMDL] && !strcasecmp(token, cmd_to_str[i][CMDL]))) {
  ...
  if (token) {
    for (i = 0; i < N_CON; i++) {
      if ((conn_to_str[i][CMDS] && !strcasecmp(token, conn_to_str[i][CMDS])) ||
          (conn_to_str[i][CMDL] && !strcasecmp(token, conn_to_str[i][CMDL]))) {
        conn = (Connector)i;
        break;
      }
    }
